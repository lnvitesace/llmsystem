    ...
    int fn_id)
{
  ...
  int ordinal = threadIdx.x + blockIdx.x * blockDim.x;
  if (ordinal < out_size){
    ...
    const int *b_strides)
{
  ...
  for (int ph = 0; ph < (n + TILE - 1) / TILE; ++ph) {
    ...
    if (row < m && a_col < n) {
        a_shared[threadIdx.y][threadIdx.x] = a_storage[
            batch * a_batch_stride + row * a_strides[1] + a_col * a_strides[2]
        ];
    } else {
        a_shared[threadIdx.y][threadIdx.x] = 0.0f;  // 越界补零
    }

    int b_row = ph * TILE + threadIdx.y;
    if (b_row < n && col < p) {
        b_shared[threadIdx.y][threadIdx.x] = b_storage[
            batch * b_batch_stride + b_row * b_strides[1] + col * b_strides[2]
        ];
    } else {
        b_shared[threadIdx.y][threadIdx.x] = 0.0f;  // 越界补零
    }
