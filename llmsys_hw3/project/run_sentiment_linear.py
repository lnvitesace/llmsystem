...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        return x.__matmul__(weights).view(batch, self.out_size) + bias
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        input = embeddings.mean(1).view(embeddings.shape[0], embeddings.shape[2])
        hidden = self.layer1.forward(input)
        hidden = minitorch.nn.dropout(hidden.relu(), self.dropout_prob)
        hidden = self.layer2.forward(hidden)
        return hidden.sigmoid().view(hidden.shape[0])
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                x, y = minitorch.tensor(X_train[example_num : example_num + batch_size], backend=BACKEND), minitorch.tensor(y_train[example_num : example_num + batch_size], backend=BACKEND)
                optim.zero_grad()
                out = model.forward(x)
