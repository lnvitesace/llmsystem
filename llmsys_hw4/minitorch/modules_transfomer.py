...
class MultiHeadAttention(Module):
    ...
    def project_to_query_key_value(self, x):
        ...
        q_proj = self.q_projection(x_flat)  # (batch_size * seq_len, n_embd)
        q = q_proj.view(batch_size, seq_len, self.n_head, self.attn_hidden_dim).permute(0, 2, 1, 3)  # (batch_size, n_head, seq_len, attn_hidden_dim)

        kT = self.k_projection(x_flat).view(batch_size, seq_len, self.n_head, self.attn_hidden_dim).permute(0, 2, 3, 1)  # (batch_size, n_head, attn_hidden_dim, seq_len)
    ...
    def self_attention(self, q, kT, v):
        ...
        if self.causal:
            ...
            scores += mask
        scores /= np.sqrt(self.attn_hidden_dim)
        attn = softmax(scores, 3) @ v
        result = attn.permute(0, 2, 1, 3).contiguous().view(batch_size, queries_len, self.n_embd)
        ### END ASSIGN3_3
    ...
    def forward(self, x):
        ...
        output = self.out_projection(attn.view(batch_size * seq_len, n_embd))  # (batch_size * seq_len, n_embd)
...
class TransformerLayer(Module):
    ...
    def forward(self, x):
        ...
        x1 = x1.view(batch_size * seq_len, n_embd) + x_2d
        x2 = self.ln_2(x1)
        x2 = self.ff(x2.view(batch_size, seq_len, n_embd))
        x2 += x1.view(batch_size, seq_len, n_embd)
        return x2
        ### END YOUR SOLUTION
...
class DecoderLM(Module):
    ...
    def forward(self, idx):
        ...
        embeddings = self.token_embeddings(idx)
        position_ids = tensor_from_numpy(np.arange(seq_len), backend=self.backend).view(1, seq_len)
        pos_embd = self.position_embeddings(position_ids).view(1, seq_len, self.n_embd)
        embeddings += pos_embd
        embeddings = self.dropout(embeddings)
        out = self.t_layer_1(embeddings)
