...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
      ...
      lib_layernorm.launch_attn_softmax_bw.argtypes = [
        np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'),
        np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'),
        ctypes.c_int,
        ctypes.c_int,
      ...
      lib_layernorm.launch_attn_softmax_bw.restype = None

      lib_layernorm.launch_attn_softmax_bw(
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      ...
      out = tensor_from_numpy(np.empty((batch_size, hidden_dim)), backend=inp.backend, requires_grad=inp.requires_grad()).contiguous()
      vars = tensor_from_numpy(np.empty(batch_size), backend=inp.backend, requires_grad=inp.requires_grad()).contiguous()
      means = tensor_from_numpy(np.empty(batch_size), backend=inp.backend, requires_grad=inp.requires_grad()).contiguous()
    ...
    def layernorm_bw(out_grad: Tensor, inp: Tensor, gamma: Tensor, beta: Tensor, var: Tensor, mean: Tensor):
      ...
      inp_grad = tensor_from_numpy(np.empty((batch_size, hidden_dim)), backend=inp.backend, requires_grad=inp.requires_grad()).contiguous()
      gamma_grad = tensor_from_numpy(np.empty((hidden_dim)), backend=inp.backend, requires_grad=inp.requires_grad()).contiguous()
      beta_grad = tensor_from_numpy(np.empty((hidden_dim)), backend=inp.backend, requires_grad=inp.requires_grad()).contiguous()
