...
class Embedding(Module):
    def __init__(self, num_embeddings: int, embedding_dim: int, backend: TensorBackend):
        ...
        self.weights = minitorch.Parameter(
            minitorch.tensor_from_numpy(np.random.normal(0, 1, (num_embeddings, embedding_dim)), backend=backend)
        )
        ### END ASSIGN3_2
    ...
    def forward(self, x: Tensor):
        ...
        bs, seq_len = x.shape
        ### BEGIN ASSIGN3_2
        x_onehot = one_hot(x, self.num_embeddings).view(bs * seq_len, self.num_embeddings)
        W = self.weights.value
        return x_onehot.__matmul__(W).view(bs, seq_len, self.embedding_dim)
        ### END ASSIGN3_2
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor: 
        ...
        if self.p_dropout == 0 or not self.training:
            return x
        if self.p_dropout == 1:
            return minitorch.zeros(x.shape)
        mask = np.random.binomial(1, 1 - self.p_dropout, x.shape)
        mask = minitorch.tensor_from_numpy(mask)
...
class Linear(Module):
    def __init__(self, in_size: int, out_size: int, bias: bool, backend: TensorBackend):
        ...
        ### BEGIN ASSIGN3_2
        def RParm(*shape):
            # Uniform initialization: Uniform(-1/sqrt(in_size), 1/sqrt(in_size))
            limit = 1.0 / (in_size**0.5)
            r = minitorch.rand(shape, backend=backend) * (2 * limit) - limit
            return minitorch.Parameter(r)

        self.weights = RParm(in_size, out_size)
        self.use_bias = bias
        if bias:
            self.bias = RParm(out_size)
        else:
            self.bias = None
        ### END ASSIGN3_2
    ...
    def forward(self, x: Tensor):
        ...
        mul = x.__matmul__(w).view(batch, self.out_size)
        if self.use_bias:
...
class LayerNorm1d(Module):
    def __init__(self, dim: int, eps: float, backend: TensorBackend):
        ...
        self.eps = eps
        ### BEGIN ASSIGN3_2
        self.weights = minitorch.Parameter(minitorch.ones((self.dim,), backend=backend))
        self.bias = minitorch.Parameter(minitorch.zeros((self.dim,), backend=backend))
        ### END ASSIGN3_2
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        batch, dim = x.shape
        ### BEGIN ASSIGN3_2
        mu = (x.sum(1) / dim).view(batch, 1)
        sigma_2 = ((x - mu) ** 2).sum(1) / dim + self.eps
        sigma = (sigma_2**0.5).view(batch, 1)
        ln_x = (x - mu) / sigma
        return self.weights.value * ln_x + self.bias.value
        ### END ASSIGN3_2
