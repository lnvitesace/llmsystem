...
__global__ void ker_layer_norm(T *ln_res, T *vars, T *means, const T *inp,
  ...
  // Step 2
  float sum = blockReduceSum<float>(l_sum);
  float square_sum = blockReduceSum<float>(l_square_sum);

  __shared__ float s_mean;
  __shared__ float s_inv_std;

  if (threadIdx.x == 0) {
    int numel = hidden_size << 2;
    float mean = sum / numel;
    float square_mean = square_sum / numel;
    float var = square_mean - mean * mean;
  ...
  for (uint idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
    float4 val = inp_f4[idx];
    float4 w = scale_f4[idx];
    float4 b = bias_f4[idx];
    float4 out;

    out.x = w.x * ((val.x - s_mean) * s_inv_std) + b.x;
    out.y = w.y * ((val.y - s_mean) * s_inv_std) + b.y;
    out.z = w.z * ((val.z - s_mean) * s_inv_std) + b.z;
    out.w = w.w * ((val.w - s_mean) * s_inv_std) + b.w;
...
__global__ void ker_ln_bw_dgamma_dbetta(T *gamma_grad, T *betta_grad,
  ...
  for (int row = threadIdx.x; row < rows; row += blockDim.x) {
    ...
    if (means != nullptr) {
      float x = (float)inp[offset];
      float mean = (float)means[row];
      float var = (float)vars[row];
      x_hat = (x - mean) * rsqrtf(var + LN_EPSILON);
    } else {
      float y = (float)inp[offset];
      float beta_val = (float)betta[col];
      float gamma_val = (float)gamma[col];
      x_hat = (y - beta_val) / gamma_val;
    }

    sum_betta += dout;
    sum_gamma += x_hat * dout;
  }
  ...
  for (int offset = g.size() / 2; offset > 0; offset >>= 1) {
    sum_betta += g.shfl_down(sum_betta, offset);
    sum_gamma += g.shfl_down(sum_gamma, offset);
  }

  if (g.thread_rank() == 0) {
    gamma_grad[col] = (T)sum_gamma;
    betta_grad[col] = (T)sum_betta;
  }
...
__global__ void ker_ln_bw_dinp(T *inp_grad, const T *out_grad, const T *inp,
  ...
  int tid = threadIdx.x;
  int bdim = blockDim.x;
  int width = hidden_dim << 2;

  __shared__ float s_mean_dxh;
  __shared__ float s_mean_dxh_xh;

  float sum_dxh = 0.0;
  float sum_dxh_xh = 0.0;

  float mu = (float)means[row];
  float rsigma = rsqrtf((float)vars[row] + LN_EPSILON);
  const T *out_grad_row = out_grad + row * width;
  const T *inp_row = inp + row * width;
  T *inp_grad_row = inp_grad + row * width;

  // Step 2
  for (int i = tid; i < width; i += bdim) {
    float dxh = (float)out_grad_row[i] * (float)gamma[i];
    float xh = ((float)inp_row[i] - mu) * rsigma;
    sum_dxh += dxh;
    sum_dxh_xh += dxh * xh;
  }
   
  // Step 3
  float sums[2] = {sum_dxh, sum_dxh_xh};
  blockReduce<ReduceType::kSum, 2>(sums);
  if (tid == 0) {
    s_mean_dxh = sums[0] / width;
    s_mean_dxh_xh = sums[1] / width;
  }
  __syncthreads();
  ...
  for (int i = tid; i < width; i += bdim) {
    float dxh = (float)out_grad_row[i] * (float)gamma[i];
    float xh = ((float)inp_row[i] - mu) * rsigma;
    inp_grad_row[i] = (T)(rsigma * (dxh - s_mean_dxh - xh * s_mean_dxh_xh));
  }
  /// END ASSIGN4_2_2
}
extern "C" {
